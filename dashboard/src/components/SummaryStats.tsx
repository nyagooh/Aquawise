...
export default function SummaryStats({ risk }: Props) {
  ...
  const stats = [
    ...
    },
    {
      label: 'Active Alerts',
      value: alerts,
      change: alerts > 0 ? 'Monitor' : 'Clear',
      changeSub: 'attention needed',
      up: false,
  ...
  return (
    <div className="grid grid-cols-2 xl:grid-cols-4 gap-5">
      {stats.map((s, i) => (
        ...
        >
          <ArrowUpRight size={16} className="absolute top-5 right-5 text-primary/30 dark:text-primary-dark/30 opacity-0 group-hover:opacity-100 transition-opacity" />
          ...
          <div className="flex items-center gap-1.5 mt-3">
            ...
            <span className="text-xs text-txt-muted dark:text-txt-dark-muted">{s.changeSub}</span>
          </div>
        </button>
      ))}
    </div>
